...
oceans_t::oceans_t()
{
    HashTable<std::shared_ptr<Pirate>>* piratesTable  = new HashTable<std::shared_ptr<Pirate>>();
    UnionFind* UF = new UnionFind();
    m_fleets = UF;
    m_pirates = piratesTable;
...
StatusType oceans_t::add_pirate(int pirateId, int fleetId)
{
    ...
    if(!fleetNode || (m_fleets->m_removed->find(fleetId)) || m_pirates->find(pirateId)){
        return StatusType::FAILURE;
    }
...
output_t<int> oceans_t::num_ships_for_fleet(int fleetId)
{
    ...
    UFnode* fleetNode = m_fleets->find(fleetId);
    if(!fleetNode || m_fleets->m_removed->find(fleetId)){
        return StatusType::FAILURE;
    }
...
StatusType oceans_t::unite_fleets(int fleetId1, int fleetId2)
{
    ...
    if (m_fleets->m_removed->find(fleetId1) || m_fleets->m_removed->find(fleetId2)){
        return StatusType::FAILURE;
    }
