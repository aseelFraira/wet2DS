...
StatusType oceans_t::add_fleet(int fleetId)
{
    ...
    if (m_fleets.m_table->find(fleetId)->m_data){
...
StatusType oceans_t::add_pirate(int pirateId, int fleetId)
{
    ...
    UFnode* fleetNode =  m_fleets.find(fleetId); // i think here we should search with find of UF

    if(!fleetNode || m_pirates.find(pirateId)){
        return StatusType::FAILURE;
    }
    std::shared_ptr<Pirate> pirate =  std::make_shared<Pirate>(pirateId);
    pirate->setArgument(fleetNode->m_fleet->getPiratesNum());
...
StatusType oceans_t::pay_pirate(int pirateId, int salary)
{
    ...
    std::shared_ptr<Pirate> pirate = m_pirates.find(pirateId)->m_data;
    if(!pirate){
        return StatusType::FAILURE;
    }
...
StatusType oceans_t::pirate_argument(int pirateId1, int pirateId2)
{
    ...
    std::shared_ptr<Pirate> pirate1 = m_pirates.find(pirateId1)->m_data;
    std::shared_ptr<Pirate> pirate2 = m_pirates.find(pirateId2)->m_data;
